...
from train.train_classifier import evaluate
...
def _write_plotly_chart(rows: list[dict], out_path: Path, model_type: str) -> None:
    ...
    try:
        ...
        log.warning(
            "Plotly not installed — skipping HTML chart. " "Install with: pip install plotly"
        )
        return
    ...
    if has_test:
        ...
        fig.add_trace(
            go.Scatter(
                x=epochs,
                y=[v * 100 if v is not None else None for v in test_t1],
                name="test top-1",
        ...
        fig.add_trace(
            go.Scatter(
                x=epochs,
                y=[v * 100 if v is not None else None for v in test_t5],
                name="test top-5",
    ...
    fig.update_layout(
        title=dict(
            text=f"{model_type} — per-epoch validation" + (" + test" if has_test else ""),
            font=dict(size=14),
        ),
        height=420,
        plot_bgcolor="white",
...
def run_evaluation(
    ...
    for _, (epoch_num, ckpt_path) in enumerate(checkpoints, 1):
    ...
    for r in sorted_rows:
        ...
        if r.get("test_top1") is not None:
            line += f"  {r['test_top1']*100:>9.2f}%" f"  {r['test_top5']*100:>9.2f}%"
        print(line + marker)
    print("─" * 72)
    print(
...
def main() -> None:
    ...
    # Output directory
    out_dir = Path(args.out_dir) if args.out_dir else Path(f"logs/{args.model}/epoch_eval")
